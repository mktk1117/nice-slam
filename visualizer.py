...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_folder", type=str, help="input folder, this have higher priority, can overwrite the one in config file"
    )
    parser.add_argument(
        "--output", type=str, help="output folder, this have higher priority, can overwrite the one inconfig file"
    )
    ...
    if args.vis_input_frame:
        ...
        frame_loader = DataLoader(frame_reader, batch_size=1, shuffle=False, num_workers=4)
    ...
    for i in tqdm(range(0, N + 1)):
        ...
        voxelfile = f"{output}/voxel/{i:05d}_voxel_hash.pt"
        if os.path.isfile(voxelfile):
            frontend.update_voxel(voxelfile)
