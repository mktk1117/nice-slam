...
class Mapper(object):
    ...
    def get_mask_from_c2w(self, c2w, key, val_shape, depth_np):
        ...
        for i in range(0, uv.shape[0], remap_chunk):
            depths += [
                cv2.remap(
                    depth_np, uv[i : i + remap_chunk, 0], uv[i : i + remap_chunk, 1], interpolation=cv2.INTER_LINEAR
                )[:, 0].reshape(-1, 1)
            ]
    ...
    def optimize_map(
        ...
        if self.nice:
            ...
            for key, val in c.items():
                if not self.frustum_feature_selection:
                    val = Variable(val.data.to(device), requires_grad=True)
                    # c[key] = val
                    if key == "grid_coarse":
        ...
        if self.nice:
            ...
            if self.BA:
                optimizer = torch.optim.Adam(
                    [{"params": decoders_para_list, "lr": 0}, {"params": camera_tensor_list, "lr": 0}]
                )
            else:
    ...
    def run(self):
        ...
        while 1:
            ...
            if not self.coarse_mapper:
                if (
                    (not (idx == 0 and self.no_log_on_first_frame)) and idx % self.ckpt_freq == 0
                ) or idx == self.n_img - 1:
                    self.logger.log(
                        idx,
                        self.keyframe_dict,
